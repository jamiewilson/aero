...
description: 'Combine client state with server fragment swaps — loading signals, adopted bindings, and shared page store.'
---
...
| Together | Actions run from reactive handlers; swapped HTML is **adopted** into the page signal store |

You do not call `adopt()` manually for standard hypermedia targets when both flags are on — the runtime runs destroy → swap → adopt for compiled regions.
...
The handler validates, persists, and returns an HTML fragment (success message or field errors). Swapped error markup with `data-aero-*` bindings is adopted automatically.
...
After swap, the paragraph stays in sync with `note` because adoption wires `$note` to the shared store.

## Manual adopt for client-inserted HTML
...
See [Adopt runtime fragments](/guide/reactivity/adopt) for structural `data-aero-switch` and other patterns.
