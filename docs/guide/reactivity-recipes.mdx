...
## Theme preference (persist + bind chain)

Keep document-level theme on a single persisted signal in the layout; pass it through components with `bind:`:
...
<html lang="en" data-theme="{ theme }">
	...
	<body>
		<nav-component bind:theme="{ theme }" />
	</body>
</html>
```
...
<!-- components/nav.html -->
<script is:state>
	const { theme = Aero.bindable() } = Aero.props
</script>
<toggle-component bind:theme="{ theme }" />
```
...
<script is:build>
	import site from '@content/site.ts'
	const themeOptions = site.theme.options
</script>
...
<script is:state>
	...
	function cycleTheme() {
		...
		theme = themeOptions[(index + 1) % themeOptions.length]
	}
</script>

<button on:click="{ cycleTheme() }">Toggle theme</button>
```

`critical: true` prepends a sync head script so `[data-theme]` CSS applies before first paint. Toggle updates the shared signal; `Aero.persist` writes `aero:theme` to `localStorage` automatically.
