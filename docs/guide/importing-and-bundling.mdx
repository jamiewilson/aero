...
**Import map (recommended):** Mark `htmx.org` and `alpinejs` as **external** in Vite so the built entry keeps those imports. Set `rolldownOptions.external` in `vite.config.ts`:
