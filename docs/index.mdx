...
| `@aero-js/core/config`  | `defineConfig` and `createViteConfig` for typed `aero.config.ts`       |
...
<Note>
	Most projects only need `@aero-js/core` and `@aero-js/core/vite` (or `@aero-js/create` to scaffold).
