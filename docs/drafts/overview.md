...
- **Conditionals and loops** — `if` / `else-if` / `else`, `for` / `data-for`, `switch` / `case` / `default` with `{ }` expressions where required. Use `<template>` for **wrapperless** branches (children only in the output); see [html-template-element.md](html-template-element.md).
