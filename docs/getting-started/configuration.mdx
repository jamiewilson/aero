...
| Flag | Enables |
| --- | --- |
| `reactivity` | `<script is:state>`, live bindings, structural updates, reactive props |
| `hypermedia` | `GET` / `POST` / … actions, fragment swaps, lifecycle events |

`busy` and post-swap binding adoption require **both** flags.

See [Reactivity](/getting-started/reactivity) and [Hypermedia](/getting-started/hypermedia) to enable and use them. HTMX and Alpine.js work without these flags — [HTMX and Alpine](/server/htmx-alpine).
