...
For imperative inserts (hypermedia swaps or `innerHTML`), adopt bindings into the page store:

```ts
import aero from '@aero-js/core'

aero.getReactivityRuntime?.()?.adopt(container)
```

See [Adopt runtime fragments](/guide/reactivity/adopt).

## Hypermedia

Reactivity alone covers client-only UI. Pair it with [Hypermedia](/getting-started/hypermedia) when fragments come from the server. `busy` and post-swap binding adoption require both flags.

## Next steps

<CardGroup cols={2}>
	<Card title="Reactivity recipes" icon="book-open" href="/guide/reactivity">
		Common patterns: forms, lists, live props, when to use Alpine instead.
	</Card>
