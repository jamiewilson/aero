...
When you enable Nitro in Aero (`aero({ server: true })`), your `server/` directory is used for API routes and optional server routes. The structure and handler API below apply to that server. Static pages are still built by Aero’s Vite pipeline; Nitro can serve them via a catch-all route (see the [kitchen-sink example](../examples/kitchen-sink)).
...
**In Aero,** static HTML is generated by the Aero/Vite build (file-based pages, `getStaticPaths`). You do not need Nitro’s prerender for that; your primary static output is `dist/` from `pnpm build`. Nitro’s **prerender** feature is for other setups or for prerendering additional routes via Nitro.

If you use Nitro’s prerender, enable it in your Nitro config:
...
Use **route rules** to control prerender and caching per route: `prerender: true`, `prerender: false`, `swr: 3600`, or `headers` for cache-control. Environment-specific handlers (e.g. `*.prerender.ts`, `*.dev.ts`) run only during prerender or in development.
