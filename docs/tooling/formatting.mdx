...
|--------|---------|-------------|
| `aeroAttributePrefix` | `false` | When `true`, emit `data-props`, `data-for`, `data-if`, … instead of bare `props`, `for`, `if`. Plain HTML attributes (e.g. `<label for="email">`) are never rewritten. |
