...
`@aero-js/core/config` already depends on `@aero-js/core` for alias resolution. Putting the CLI inside core and importing config loading would risk a circular dependency. The CLI lives in `packages/cli`, depends on `@aero-js/core`, `@aero-js/core/config`, and `@aero-js/content`, and publishes the `aero` binary.
