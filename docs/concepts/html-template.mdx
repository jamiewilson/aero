...
Put `switch` / `aero-switch` on **`<template>`** to match a discriminant against **`case`** branches without emitting an extra wrapper. Direct children of the container must be branch elements (`case` or `default`); see [Props](/data/props) for expression rules.
