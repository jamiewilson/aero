...
The `fullstack` template also adds Nitro, a root `nitro.config.ts`, and a `preview:api` command. Templates ship inside this package under `templates/` (`minimal`, `fullstack`).
