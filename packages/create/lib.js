 ...
 * @param {string} templatePath - Path to the template directory (e.g. packages/create/templates/minimal)
...
export function writeReadme(targetDir, projectName, template) {
	if (template === 'fullstack') {
		const lines = [
			...
			'├── server/             # Nitro API/routes, plugins, tasks, and entry',
			'│   ├── api/            # Nitro API handlers',
			'│   ├── routes/         # Nitro route handlers (e.g. static catch-all)',
			'│   ├── plugins/        # Nitro runtime hooks',
			'│   ├── tasks/          # Nitro task handlers',
			'│   └── entry.ts        # Nitro server entry (wired via serverEntry)',
			...
			'- Nitro plugin hooks in `server/plugins/`',
			'- Nitro tasks in `server/tasks/`',
			'',
			'## Learn More',
			'',
