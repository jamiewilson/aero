...
import { isOffsetInRanges, collectInterpolationBodyRanges, prepareAeroTemplateSource } from '@aero-js/interpolation'
...
export function parse(html: string): ParseResult {
	...
	const { htmlSafeText: escapedHtml, restore } = prepareAeroTemplateSource(html)
