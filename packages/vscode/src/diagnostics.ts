...
export class AeroDiagnostics implements vscode.Disposable {
	...
	private updateDiagnostics(document: vscode.TextDocument): void {
		...
		this.checkComponentProps(document, text, diagnostics)
	...
	private getAttributeKeysFromTag(attrs: string): string[] {
		const keys: string[] = []
	 ...
	 */
	private traceLayoutToSinkProps(
		layoutPath: string,
		...
		for (let i = 0; i < 10; i++) {
			...
			const layoutDir = currentPath.replace(/\/[^/]+$/, '')
			// Find child that receives props: <base-layout props> or <meta-component props="{ ...Aero.props }" />
