...
function buildFetchOptions(request: HypermediaRequest): RequestInit {
...
export async function executeRequest(request: HypermediaRequest): Promise<HypermediaResponse> {
	const response = await fetch(request.url, buildFetchOptions(request))
