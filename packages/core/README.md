...
| State       | `<script is:state>`    | Browser           | Requires `reactivity: true`. Declares signals and reactive bindings.                       |
...
- Supports components, slots, `for` / `for`, `if` / `else-if` / `else`, and `props` on scripts and styles.
...
- **Plugin** from `@aero-js/core/vite`: `aero(options?)`. Options: `content`, `server`, `reactivity`, `hypermedia`, `apiPrefix`, `dirs`, `site`, `redirects`, `middleware` (`AeroOptions`).
