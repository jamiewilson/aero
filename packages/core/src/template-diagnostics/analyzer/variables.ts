...
function bindingToVariableDefinition(
	...
	const def: VariableDefinition = {
		name: b.name,
		range: rangeFromOffsets(document, contentStart + b.start, contentStart + b.end),
		kind,
	}
...
export function collectDefinedVariables(
	...
	Array<{ name: string; kind1: string; kind2: string; range: VariableDefinition['range'] }>,
] {
...
function addPropsInjectedVars(
	...
	for (const name of parsed.injectedNames) {
		...
		setVar(name, {
			name,
			range: rangeFromOffsets(document, rangeStart, rangeStart + name.length),
			kind: 'reference',
		})
	}
}
...
export function collectVariablesByScope(
	...
	const buildBindingProperties =
		scope === 'build'
			...
			: buildBindingPropertiesFromVars(collectDefinedVariables(document, text)[0])
